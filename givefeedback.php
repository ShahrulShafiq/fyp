  ...
  if(isset($_POST['rating'])){
      ...
      $id = $_GET['id'];
      $selectvendor = "SELECT * FROM vendors where id = $id";
      $fetchvendor = mysqli_query($mysqli,$selectvendor);
      $resultvendor = mysqli_fetch_assoc($fetchvendor);
      $vendorname = $resultvendor['businessname'];

      
      $checkuser = "SELECT * FROM feedback WHERE username = '$username' AND order_code = '$ordercode' AND product_code = '$productcode'";
      $userquery = mysqli_query($mysqli,$checkuser);
      $usercount = mysqli_num_rows($userquery);

      if($usercount>0){
        $msg = "<div class='alert alert-danger'>You already give a feedback</div>";
      }else{
      $query = "INSERT INTO `feedback`(`rating`, `title`, `feedback`, `user_id`,`username`, `user_profile` ,`vendor_name`,`order_code`,`product_code`,`date`) VALUES(?,?,?,?,?,?,?,?,?,?)";
      $stmt = $mysqli->prepare($query);
      $stmt -> bind_param ('ississssss',$rating, $title, $feedback, $userid, $username, $profilepic, $vendor,$ordercode,$productcode,$date);
      $stmt-> execute();
      $msg = "<div class='alert alert-success'>Review Added</div>";
      $stmt->close();
}
  } 
    ...
    <div class="col-md-8 col-xl-9">
        <div class="card">
            ...
            <div class="card">
                ...
                <li class="nav-item"><a href="rating.php?id=<?php echo $result['id']; ?>" class="text-dark nav-link">Ratings & Feedback</a></li>
                <li class="nav-item"><a href="itembought.php?id=<?php echo $result['id']; ?>" class="nav-link active font-weight-bold">Item Purchased</a></li>
              </ul>
            ...
            <?php if(isset($msg)){echo $msg;} ?>
             <form action="" method="post">
              ...
              $ambilnamaproduct = "SELECT * FROM product WHERE product_code ='$productcode'";
              $queryproduct = mysqli_query($con,$ambilnamaproduct);
              $resultproduct = mysqli_fetch_assoc($queryproduct);
              $namaproduct = $resultproduct['product_name'];
              ?>
              <div class="form-group">
             
              <input type="text" class="form-control" name="title" placeholder="Title" value="<?php echo $namaproduct ?>" hidden>
              <!-- important 1 -->

              <input type="text" name="date" value="<?php echo date("Y-m-d") ?>" hidden>

              <input type="hidden" class="form-control" name="vendor" value="<?php echo $result['businessname']; ?>">
              <input type="hidden" class="form-control" name="profilepic" value="<?php echo $_SESSION['user_profile']; ?>">
              <input type="hidden" class="form-control" name="username" value="<?php echo $_SESSION['username']; ?>">
              </div>
              <div class="form-group">
              
              <textarea type="text" class="form-control" name="feedback" rows="5" id="comment" placeholder="Feedback" required></textarea>
